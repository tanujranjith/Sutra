...
| `features/` | Per-feature guides: daily-loop Today, adaptive navigation/mobile behavior, Homework/Course Hub/Review/Timeline, Canvas, Slides, Assistant, encrypted Sutra Sync, cloud backup, academic planning, document backgrounds, handwriting, mods & customization, CSS mods, plugin SDK, landing surfaces, brand assets, quotes, and rebrand compatibility. |
...
- The in-app **Help & Docs** page is generated by the classic runtime and
  reconciled by `src/features/workspace/help-docs-refresh.js`; keep it aligned
  with `README.md`, `SUTRA_GUIDE.md`, the Product Knowledge registry, and the
  feature guides when user-facing behavior changes.
