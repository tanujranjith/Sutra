(function (global) {
  ...
  function moveElementDrag(event) {
    ...
    else { element.x = Math.max(0, Math.min(100 - dragState.original.width, dragState.original.x + dx)); element.y = Math.max(0, Math.min(100 - dragState.original.height, dragState.original.y + dy)); }
  ...
  function mount() {
    if (root) return root;
    root = document.createElement('section'); root.id = 'slidesEditor'; root.className = 'slides-editor'; root.hidden = true; root.setAttribute('inert', ''); root.setAttribute('aria-hidden', 'true'); root.setAttribute('aria-label', 'Slides editor');
    root.innerHTML = '<div class="slides-mode-tabs" role="tablist" aria-label="Notes mode"><span>Notepad</span><span>Canvas</span><strong aria-selected="true">Slides</strong></div><div class="slides-toolbar" role="toolbar"><div class="slides-toolbar-main"></div><div class="slides-toolbar-end"></div></div><div class="slides-workspace"><aside class="slides-thumbnails" aria-label="Slides"><div class="slides-thumbnail-list"></div><button type="button" class="slides-add-thumbnail" aria-label="Add slide"><i class="fas fa-plus" aria-hidden="true"></i></button></aside><main class="slides-center"><div class="slides-stage" tabindex="0" aria-label="Editable current slide"></div><section class="slides-notes-panel"><label><i class="fas fa-note-sticky" aria-hidden="true"></i> Speaker notes</label><textarea aria-label="Speaker notes"></textarea></section></main><aside class="slides-inspector"><h3>Design</h3><label>Theme<select data-theme><option value="sutra">Sutra</option><option value="nature">Sutra Nature</option><option value="midnight">Midnight</option><option value="paper">Paper</option></select></label><label>Layout<select data-layout><option value="title">Title</option><option value="title-body">Title &amp; body</option><option value="two-column">Two column</option><option value="three-card">Title &amp; three cards</option><option value="image-caption">Image &amp; caption</option><option value="blank">Blank</option></select></label><label>Slide size<select data-size><option value="widescreen">16:9 (Widescreen)</option><option value="standard">4:3 (Standard)</option></select></label><label class="slides-toggle"><input type="checkbox" data-notes checked> Show speaker notes</label><section class="slides-element-inspector" data-element-inspector hidden><h3>Selected object</h3><p data-element-name></p><label>Text size<input type="range" min="1" max="10" step=".5" data-element-font></label><label class="slides-toggle"><input type="checkbox" data-element-bold> Bold text</label><label>Text color<input type="color" data-element-color></label><label>Fill color<input type="color" data-element-fill></label><div class="slides-inspector-grid"><button type="button" data-element-back>Send back</button><button type="button" data-element-forward>Bring forward</button><button type="button" data-element-copy>Copy object</button><button type="button" data-element-duplicate>Duplicate object</button><button type="button" data-element-delete>Delete object</button></div></section><button type="button" data-export-pdf>Print / PDF</button><button type="button" data-export-pptx>Export PPTX</button></aside></div><div class="slides-status"><span data-count></span><span>Saved locally</span><button type="button" data-present><i class="fas fa-play" aria-hidden="true"></i> Present</button></div>'; // sutra-allow-html: reviewed static Slides workbench chrome; deck content is assigned with textContent.
    var container = document.getElementById('notesPrimaryPane'); if (container) container.appendChild(root);
    var toolbar = root.querySelector('.slides-toolbar-main'); toolbar.append(button('New Slide', 'fa-plus', function () { mutate(function (deck) { var next = makeSlide('title-body', 'New slide'); deck.slides.push(next); activeSlideId = next.id; }); })); toolbar.append(button('Undo', 'fa-undo', slidesUndo)); toolbar.append(button('Redo', 'fa-redo', slidesRedo)); toolbar.append(button('Text', 'fa-font', function () { addElement('text'); })); toolbar.append(button('Shape', 'fa-shapes', function () { addElement('shape'); })); toolbar.append(button('Image', 'fa-image', chooseImage)); toolbar.append(button('Chart', 'fa-chart-bar', function () { addElement('chart'); }));
